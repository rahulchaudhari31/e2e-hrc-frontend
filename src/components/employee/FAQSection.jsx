...
import { getEmployeeFAQs } from '../../services/employee/employeeFAQService';
import { getEmployeeCTA } from '../../services/employee/employeeCTAService';
...
export default function FAQSection() {
  ...
  useEffect(() => {
    const loadFAQs = async () => {
      try {
        const items = await getEmployeeFAQs();
        setFaqs(items);
      } catch (error) {
        setFaqError(true);
      } finally {
    ...
    const loadCTA = async () => {
      try {
        const activeCta = await getEmployeeCTA();
        setCta(activeCta);
      } catch (error) {
        setCtaError(true);
      } finally {
  ...
  const faqContent = loadingFAQs ? (
    ...
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-8 text-center">
      <p className="text-gray-500">Employee FAQs are not available at the moment.</p>
  ...
  const ctaContent = loadingCTA ? (
    ...
    <div className="relative w-full max-w-xl rounded-3xl p-8 bg-linear-to-br from-blue-700 to-blue-900 text-white shadow-2xl">
      <div className="space-y-6 pr-4">
        <div className="space-y-3">
          <h2 className="text-3xl sm:text-4xl font-bold leading-tight">Employee CTA</h2>
    ...
    <div className="relative w-full max-w-xl bg-linear-to-br from-blue-700 to-blue-900 rounded-3xl p-8 text-white shadow-2xl">
      ...
      <div className="space-y-6 pr-4">
        ...
        {cta.buttonLink && cta.buttonLink.startsWith('/') ? (
          <Link
            to={cta.buttonLink}
