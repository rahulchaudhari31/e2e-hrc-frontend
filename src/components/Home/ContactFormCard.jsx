...
import { Phone, Mail, User, Paperclip, ChevronDown, Search, Building2, Briefcase } from "lucide-react";
...
const CONFIG = {
  employee: {
    heading: "Get in Touch with Our Employee Team",
    description:
    ...
    submitLabel: "Submit",
    attachmentPlaceholder: "Upload your resume / CV",
    fields: ["name", "email", "contact", "attachment"],
  },
  employer: {
    heading: "Submit a Vacancy",
    description:
      "Looking to hire exceptional talent? Fill out the form below with your vacancy details and our team will get back to you shortly.",
    iam: "employer",
    subject: "Vacancy Submission",
    submitLabel: "Submit Vacancy",
    attachmentPlaceholder: "Upload job description (optional)",
    fields: ["name", "email", "contact", "company", "jobTitle", "attachment"],
  },
};

const ContactFormCard = ({ type = "employee" }) => {
  const cfg = CONFIG[type] || CONFIG.employee;
  const showCompany = cfg.fields.includes("company");
  const showJobTitle = cfg.fields.includes("jobTitle");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      payload.append(
        "company",
        showCompany && formData.company.trim()
          ? formData.company.trim()
          : "Not specified"
      );
      payload.append("email", formData.email.trim());
      payload.append("iam", cfg.iam);
      ...
      payload.append(
        ...
        `Contact Number: ${selectedCountry.dial} ${formData.contactNumber.trim()}${
          showCompany && formData.company.trim()
            ...
            : ""
        }${showJobTitle && formData.jobTitle.trim() ? `\nJob Title: ${formData.jobTitle.trim()}` : ""}`
      );
      ...
      toast.success(
        type === "employer"
          ? "Your vacancy has been submitted. Our team will get back to you shortly."
  ...
  const inputBase =
    "w-full rounded-full border border-gray-200 bg-white px-4 py-3 text-sm text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#0b3a91]/30 focus:border-[#0b3a91]";
  const iconWrap = "absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none";

  return (
    <>
      <div className="bg-white rounded-3xl shadow-2xl p-6 sm:p-8 w-full max-w-md mx-auto h-[560px] flex flex-col">
        <div className="flex items-start justify-between mb-3 shrink-0">
          ...
          <div className="shrink-0">
            <img
              src={groupIcon}
              alt=""
              style={{ width: "80px", height: "60px", opacity: 1 }}
            />
          </div>
        </div>
        ...
        >
          <div>
            ...
            <div className="relative">
              <User className={`${iconWrap} w-4 h-4 text-gray-400`} />
              <input
                id="name"
                name="name"
                type="text"
                ...
                className={inputBase.replace("px-4", "pl-11 pr-4")}
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <Mail className={`${iconWrap} w-4 h-4 text-gray-400`} />
              <input
                id="email"
                name="email"
                type="email"
          ...
          {showCompany && (
            <div>
              ...
              <div className="relative">
                <Building2
                  className={`${iconWrap} w-4 h-4 text-gray-400`}
                />
                <input
                  id="company"
                  name="company"
          ...
          <div>
            <label className="block text-sm font-semibold text-[#0b3a91] mb-1.5">
              Attachment
            </label>
            <label className="flex items-center gap-3 w-full rounded-full border border-gray-200 px-4 py-3 text-sm text-gray-400 cursor-pointer hover:border-gray-300 transition-colors">
              <Paperclip className="w-4 h-4 text-gray-400 shrink-0" />
              <span className="truncate">
                {formData.attachment
                  ? formData.attachment.name
                  : cfg.attachmentPlaceholder}
              </span>
              <input
                id="attachment"
                name="attachment"
